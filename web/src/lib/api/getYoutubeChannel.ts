...
const Video = z
  ...
  .transform((args) => {
    ...
    return {
      id:
        args.id.kind == 'youtube#video'
          ? args.id.videoId
          : args.id.kind === 'youtube#playlist'
            ? args.id.playlistId
            : args.id.channelId,
      kind: args.id.kind,
...
export async function getVideos() {
  try {
    const res = await fetch(
      `https://www.googleapis.com/youtube/v3/search?key=${API_KEY}&channelId=UChYjCtGvtgzh7HvJuanV1-g&part=snippet,id&order=date&maxResults=100`
    );
    const data = await res.json().then((r) => {
      const result = Videos.safeParse(r.items);
      if (result.success) {
        return result.data;
      }
