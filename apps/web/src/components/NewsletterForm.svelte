<script lang="ts">
	...
	const id = lang === 'es' ? 'c4r8t8' : 'h7k7g0';
</script>

<section class="font-body pt-8  dark:text-white text-center">
	<p>
		{$t('common.newsletter_text')}
	</p>
	<form
		class="flex flex-col md:flex-row gap-4 justify-between mx-auto mt-4 p-8 prose prose-lg items-center"
		action={`https://static.mailerlite.com/webforms/submit/${id}`}
		data-code={id}
		method="post"
		target="_blank"
	>
		...
		<input
			...
			class="px-4 font-medium rounded-md show-md w-full md:w-80 shadow-md"
			autocomplete="email"
		/>
		<button
			type="submit"
			class={`h-8 w-full md:w-52 
bg-ebony-clay-600 
dark:bg-gray-200 
...
rounded-full shadow-md group`}
 ...
 transition-all duration-300 transform group-hover:translate-x-full ease`}
				>{$t('common.join_now')}</span
			>
			<span class="relative invisible">{$t('common.join_now')}</span>
		</button>
	</form>
</section>
