...
<form
	class="text-center mt-2 text-ebony-clay-800 mx-auto w-4/5"
	on:submit|preventDefault={sendIssue}
>
	...
	<div class="text-xs mt-0 mb-2 font-bold uppercase">
		<i class="fas fa-map-marker-alt mr-2 opacity-75" />{$t('about.issue')}
	</div>
	<div class="flex flex-col gap-6 mt-8">
		<input
			type="text"
			placeholder="Title"
			class="text-sm border-slate-300 hover:border-slate-400 focus:border-border-slate-400 focus:bg-secondary px-6 py-4 w-full bg-white border rounded-lg focus:outline-none"
			name="title"
			required
		/>
		<input
		...
		<button
			type="submit"
			class="mt-6 text-gray-200 font-bold py-2 px-4 rounded-lg hover:opacity-80 focus:opacity-70 focus:outline-none bg-[#6366F1] disabled:opacity-60 disabled:cursor-not-allowed"
			disabled={creatingIssue || question.length <= 0}
		>
			<span class="text-sm"
				>{#if creatingIssue} {$t('about.publishing')} {:else} {$t('about.send')}{/if}</span
			>
		</button>
		<div
			id="hcaptcha"
		...
		<p class="text-sm text-ebony-clay-800 ">
			This site is protected by hCaptcha and its <a href="https://www.hcaptcha.com/privacy"
				>Privacy Policy</a
			>
